...
class Team_members extends Security_Controller
{
    ...
    public function teste()
    {
        ...
        return $this->template->rander("team_members/teste", $view_data);
    }
    ...
    function get_all_subordinates($user_id, $custom_fields)
    {
        $options = array(
        ...
        $subordinates = $this->Users_model->get_details($options)->getResult();
        $result = array();
        foreach ($subordinates as $subordinate) {
            $result[] = $this->_make_row($subordinate, $custom_fields);

            // Call this function recursively to get all subordinates of the current subordinate.
            $subordinate_subordinates = $this->get_all_subordinates($subordinate->id, $custom_fields);
            $result = array_merge($result, $subordinate_subordinates);
        }
    ...
    function list_data()
    {
        ...
        $result = $this->get_all_subordinates($this->login_user->id, $custom_fields);
    ...
    {
        $piccoin_balance = new Piccoin_model();
        $balance = $piccoin_balance->getPiccoinData($data->id);
        $pic_balance = $balance['balance'];
        ...
        if ($show_contact_info) {
            $transfer_form = form_open('.com.br/index.php/transaction/transfer', ['class' => 'transfer valueForm', 'id' => 'valueForm']);
            $transfer_form .= form_input(array(
                'type'  => 'hidden',
                'name'  => 'receiver_id',
                'value' => $data->id
            ));
            $transfer_form .= form_input(array(
                'type'        => 'number',
                'name'        => 'amount',
                'placeholder' => 'Digite o valor para transferir',
                'class'       => 'form-control'
            ));
            $transfer_form .= form_submit('submit', 'Transferir', 'class="btn btn-primary"');
            $transfer_form .= form_close();
        }

        $row_data[] = $transfer_form;
    ...
    //show integration of a team member
    function integration($user_id)
    {
        validate_numeric_value($user_id);
        $this->update_only_allowed_members($user_id);
        ...
        $getSecretKey = new Api_secretkey_model();
        $view_data["secretkey"] = $getSecretKey->getSecretKeys($user_id);

        return $this->template->view("team_members/integration", $view_data);
    }
